...
class EventUpdateManager(private val activityContext: WorkManagerActivity) {
    ...
    fun exec() {
        ...
        WorkManager.getInstance(activityContext).getWorkInfoByIdLiveData(workReq.id)
            .observe(activityContext, observerEventUpdate)
    }
    fun cancelAndRestart() {
        ...
        WorkManager.getInstance(activityContext).enqueue(workReq1)
        WorkManager.getInstance(activityContext).getWorkInfoByIdLiveData(workReq1.id)
            .observe(activityContext, observerEventUpdate)
    }
    private val observerEventUpdate: (WorkInfo) -> Unit = { workInfo ->
        when (workInfo.state) {
            WorkInfo.State.SUCCEEDED -> {
                ...
                event?.let {
                    val msg = activityContext.getString(R.string.successfully_updated_event)+" event = $it"
